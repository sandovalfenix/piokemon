<template>
  <div class="gym-zone">
    <div class="gym-header">
      ...
      <div class="gym-info">
        <h1>Plazoleta Jairo Varela</h1>
        <p>Tipo: Música/Eléctrico</p>
      </div>
      <div class="stats-bar">
    ...
    <div class="gym-map-container">
      <div ref="mapCanvas" class="map-canvas">
        <img ref="mapImg" :src="zoneImage" alt="Plazoleta Jairo Varela" class="map-image" />
        ...
        <div class="npc trainer" :style="trainer1Style" v-if="!trainersDefeated.trainer1">
          <div class="npc-sprite"><img :src="playerImage" alt="Bailarín Luis" class="npc-image small" /></div>
          <div class="npc-label trainer">Bailarín Luis</div>
        </div>

        <div class="npc trainer" :style="trainer2Style" v-if="!trainersDefeated.trainer2">
          <div class="npc-sprite"><img :src="playerImage" alt="DJ Camila" class="npc-image small" /></div>
          <div class="npc-label trainer">DJ Camila</div>
        </div>
      </div>
    </div>
    ...
    <div v-if="currentEncounter?.type === 'pokemon'" class="modal-overlay">
      <div class="modal pokemon-modal">
        ...
        <div class="modal-content">
          ...
          <p class="encounter-text">Un {{ currentEncounter.data.name }} danzaba al ritmo del Salsódromo.</p>
        </div>
...
import zoneImage from '../assets/plazoletavarela.png'
...
onMounted(() => { mapStore.unlockNextZone(1) })

const wildPokemon = [
  { id: 1, name: 'Plusle', level: 20, found: false },
  { id: 2, name: 'Minun', level: 20, found: false },
]

const items = [
  { id: 1, name: 'Batería', description: 'Recarga energía de aparatos.', found: false },
  { id: 2, name: 'Revive', description: 'Revive a un Pokémon con la mitad de PS.', found: false },
]
...
const trainers = {
  trainer1: { name: 'Bailarín Luis', dialogue: 'El ritmo manda.', pokemon: 'Voltorb', pokemonLevel: 19, position: { x: 40, y: 60 } },
  trainer2: { name: 'DJ Camila', dialogue: 'Sube el volumen.', pokemon: 'Electrike', pokemonLevel: 21, position: { x: 70, y: 35 } },
}
...
const { position: playerPosition, isMoving, move } = useKeyboardMovement({
  ...
  onMove: () => {
    if (!currentEncounter.value) { stepCount.value++; checkRandomEncounter() }
  },
})
...
<style scoped>
.gym-zone{width:100%;min-height:100vh;background:linear-gradient(135deg,#263238 0%,#546e7a 50%,#263238 100%);position:relative;overflow:hidden}
